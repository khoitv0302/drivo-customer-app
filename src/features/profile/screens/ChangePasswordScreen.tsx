  ...
  function handleSubmit() {
    ...
    changePassword(
      { currentPassword: current, newPassword: next },
      {
        ...
        onError: (err) => {
          // Ưu tiên lỗi theo field từ backend; nếu không có thì hiện lỗi chung.
          const cur = err.fieldError('currentPassword');
          const nw = err.fieldError('newPassword');
          if (cur || nw) setErrors({ current: cur, next: nw });
    ...
    >
      ...
      >
        ...
        <PasswordField
          label="Mật khẩu mới"
          value={next}
          onChange={v => { setNext(v); setErrors(e => ({ ...e, next: undefined, general: undefined })); }}
          error={errors.next}
        ...
        <PasswordField
          ...
          value={confirm}
          onChange={v => { setConfirm(v); setErrors(e => ({ ...e, confirm: undefined, general: undefined })); }}
        ...
        {errors.general ? (
          <View className="flex-row items-center bg-red-50 rounded-xl px-3 py-2.5 mb-4">
            ...
            <Text className="text-sm text-red-500 ml-2 flex-1">{errors.general}</Text>
          </View>
        ) : null}
